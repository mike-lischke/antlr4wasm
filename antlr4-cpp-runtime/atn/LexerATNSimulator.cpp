...
size_t LexerATNSimulator::execATN(CharStream *input, dfa::DFAState *ds0) {
  std::cout << "start execATN()" << std::endl;
  if (ds0->isAcceptState) {
    // allow zero-length tokens
    // ml: in Java code this method uses 3 params. The first is a member var of the class anyway (_prevAccept), so why
    // pass it here?
...
size_t LexerATNSimulator::failOrAccept(CharStream *input, ATNConfigSet *reach, size_t t) {
  if (_prevAccept.dfaState != nullptr) {
    accept(input, _prevAccept.dfaState->lexerActionExecutor, _startIndex, _prevAccept.index, _prevAccept.line,
           _prevAccept.charPos);
  ...
  for (const auto &c : closure_->configs) {
    ...
    for (size_t ti = 0; ti < n; ti++) { // for each transition
      ...
      if (target != nullptr) {
        ...
        Ref<LexerATNConfig> config = std::make_shared<LexerATNConfig>(downCast<const LexerATNConfig &>(*c), target,
...
void LexerATNSimulator::accept(CharStream *input, const Ref<const LexerActionExecutor> &lexerActionExecutor,
...
dfa::DFAState *LexerATNSimulator::addDFAState(ATNConfigSet *configs, bool suppressEdge) {
  ...
  if (firstConfigWithRuleStopState != nullptr) {
    ...
    proposed->lexerActionExecutor =
      downCast<const LexerATNConfig &>(*firstConfigWithRuleStopState).getLexerActionExecutor();
